...
class SampleDisplay(QFrame):
    ...
    def _setup_ui(self, title: str):
        ...
        max_height = 120 if self.compact else 140
...
class QtResultPreview(QWidget):
    ...
    def _setup_ui(self):
        ...
        # === 左侧：样例区域（垂直排列，带折叠） ===
        left_panel = QWidget()
        left_panel.setFixedWidth(280)  # 固定宽度
        ...
        # 手工表样例（可折叠）
        self.manual_sample = SampleDisplay("手工表样例", "#e3f2fd", compact=True)
        left_layout.addWidget(self.manual_sample)
        
        # 系统表样例（可折叠）
        self.system_sample = SampleDisplay("系统表样例", "#e8f5e9", compact=True)
        left_layout.addWidget(self.system_sample)
        ...
        # 标题
        preview_title = QLabel("📊 对账预览")
        preview_title.setFont(QFont("Microsoft YaHei", title_size, QFont.Weight.Bold))
        ...
        formula_layout.setContentsMargins(6, 6, 6, 6)
        ...
        right_layout.addWidget(formula_frame)
        ...
        header_padding = "4px" if self.compact else "6px"
        ...
        layout.addWidget(right_panel, 1)  # 右侧面板占据剩余空间
