...
class ResultPreview(ttk.Frame):
    ...
    def update_preview(
        ...
        style.map("Treeview.Heading", background=[("active", "#223447")])

        # 预先设置 tag 颜色（在插入前）
        self.tree.tag_configure("match", background="#12312b", foreground="#b7f5d0")
        self.tree.tag_configure("diff", background="#3a3216", foreground="#f4e3b0")
        self.tree.tag_configure("missing", background="#3a1f26", foreground="#f6c1cb")
