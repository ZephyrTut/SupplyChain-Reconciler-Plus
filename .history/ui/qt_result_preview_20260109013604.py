...
class SampleDisplay(QFrame):
    ...
    def set_data(self, df: pd.DataFrame, key_cols: List[str], value_col: str, 
                 total_count: int, pivot_info: str = "", show_all_cols: bool = False):
        """设置数据（显示筛选后的数据样例）
        ...
        Args:
            ...
            total_count: 总数据量
            pivot_info: 透视信息
            show_all_cols: 是否显示所有列（筛选后的原始数据）
        """
        lines = []
        
        if show_all_cols and not df.empty:
            # 显示所有列（横向滚动）
            cols_to_show = list(df.columns)[:8]  # 最多显示8列
            for i, (_, row) in enumerate(df.head(5).iterrows()):
                parts = [f"{str(row.get(col, ''))[:12]}" for col in cols_to_show]
                line = " | ".join(parts)
                if len(df.columns) > 8:
                    line += " ..."
...
class QtResultPreview(QWidget):
    ...
    def _apply_filter(self, df: pd.DataFrame, col: str, op: str, val: str) -> pd.DataFrame:
        """应用单个筛选条件（与引擎逻辑一致）"""
        ...
        if op == "EQUALS":
            ...
            values = [v.strip() for v in val.replace('；', ';').replace('，', ',').replace(';', ',').split(',') if v.strip()]
            return df[col_data.isin(values)]
        elif op == "NOT_IN_LIST":
            values = [v.strip() for v in val.replace('；', ';').replace('，', ',').replace(';', ',').split(',') if v.strip()]
            return df[~col_data.isin(values)]
        elif op == "GREATER":
    ...
    def update_preview(self, manual_df: pd.DataFrame, system_df: pd.DataFrame, 
        ...
        try:
            ...
            # 应用筛选条件到原始数据（用于样例显示）
            manual_filtered = manual_with_key.copy()
            system_filtered = system_with_key.copy()
            
            # 手工应用筛选（与引擎逻辑一致）
            for col, op, val in manual_filters:
                if col in manual_filtered.columns:
                    manual_filtered = self._apply_filter(manual_filtered, col, op, val)
            for col, op, val in system_filters:
                if col in system_filtered.columns:
                    system_filtered = self._apply_filter(system_filtered, col, op, val)
            
            # 更新样例（显示筛选后的原始数据，包含所有列）
            self.manual_sample.set_data(
                manual_filtered, manual_keys, manual_value, len(manual_filtered),
                show_all_cols=True
            )
            self.system_sample.set_data(
                system_filtered, system_keys, system_value, len(system_filtered), 
                pivot_info, show_all_cols=True
            )
