...
class ColumnCleanRow(DynamicRow):
    ...
    def _setup_ui(self):
        ...
        self.column_combo.setMaximumWidth(140)
        layout.addWidget(self.column_combo)
        
        # 预设规则复选框（紧凑排列）
        for preset_name, _ in self.CLEAN_PRESETS:
            cb = QCheckBox(preset_name)
            cb.setStyleSheet("QCheckBox { color: #333; font-size: 11px; spacing: 2px; }")
        ...
        self.custom_edit.setPlaceholderText("自定义去除")
        self.custom_edit.setFixedWidth(80)
    ...
    def set_value(self, value: Dict):
        ...
        if match:
            self.custom_edit.setText(match.group(1))
                    break
        if "custom" in value:
            self.custom_edit.setText(value["custom"])
