...
class FilterRow(DynamicRow):
    ...
    OPERATORS = ["等于", "不等于", "包含", "不包含", "包含于", "大于", "小于"]
    ...
    def _setup_ui(self):
        ...
        self.value_combo.setPlaceholderText("输入值（多个值用逗号分隔）")
    ...
    def _on_operator_changed(self):
        ...
        if operator == "包含于":
            self.value_combo.setPlaceholderText("多个值用逗号分隔，如: 发货,退仓,退货")
        else:
            self.value_combo.setPlaceholderText("输入值")
        self.changed.emit()
...
class QtConfigPanel(QScrollArea):
    ...
    def update_formula_options(self, column_letters: Dict[str, str]):
        ...
        Args:
            column_letters: 列名到字母的映射，如 {"__KEY__": "A", "透视列1": "B", ...}
        """
        # 保存当前公式
        ...
        # 解析字母映射
        manual_letter = "M"  # 手工数量默认用M
        system_letter = "S"  # 系统总计默认用S
        ...
        if pivot_letters:
            ...
            formula_options.append(
                f"排除{first_name}: {manual_letter} - ({system_letter} - {first_letter}) (排除特定透视列)"
            )
            ...
            if len(pivot_letters) >= 2:
                ...
                formula_options.append(
                    f"特定透视: {manual_letter} - ({first_letter} + {second_letter}) (只对比指定列)"
                )
            ...
            if len(pivot_letters) >= 3:
                ...
                if len(pivot_letters) > 5:
                    pivot_sum += " + ..."
                formula_options.append(
                    f"透视汇总: {manual_letter} - ({pivot_sum}) (对比所有透视列)"
                )
