...
class QtConfigPanel(QScrollArea):
    ...
    def _create_formula_section(self):
        ...
        self.formula_edit.setText("B - C")  # 默认：手工数量(B) - 系统总计(C)
        self.formula_edit.setPlaceholderText("例如: B - C, B - (F - C)")
        ...
        # 变量说明
        self.formula_hint = QLabel("B=手工数量, C=系统总计")
    ...
    def update_formula_options(self, column_letters: Dict[str, str]):
        ...
        # 同时更新公式输入框为默认公式（如果当前是M-S或为空）
        current = self.formula_edit.text().strip()
        if not current or current == "M - S":
    ...
    def get_config(self) -> Dict[str, Any]:
        ...
        # 差值公式（使用动态字母，默认B - C）
        config["difference_formula"] = self.formula_edit.text().strip() or "B - C"
