...
class ResultPreview(ttk.Frame):
    ...
    def __init__(self, parent):
        ...
        # 手工表样例区域
        manual_outer = ttk.Frame(samples_frame, bootstyle="info", padding=5)
        ...
        manual_scrollbar = ttk.Scrollbar(manual_scroll_container, orient="vertical",
                         command=self.manual_canvas.yview, bootstyle="secondary-round")
        ...
        system_scrollbar = ttk.Scrollbar(system_scroll_container, orient="vertical",
                         command=self.system_canvas.yview, bootstyle="secondary-round")
        ...
        y_scroll = ttk.Scrollbar(table_container, orient="vertical", command=self.tree.yview, bootstyle="secondary-round")
        x_scroll = ttk.Scrollbar(table_container, orient="horizontal", command=self.tree.xview, bootstyle="secondary-round")
    ...
    def _show_placeholder(self):
        ...
        ttk.Label(self.manual_scroll_frame, text="配置后显示", 
             font=UI_FONT_SMALL, bootstyle="secondary").pack(pady=5)
    ...
    def update_preview(
        ...
        # 预先设置 tag 颜色（在插入前）- 使用柔和的淡色系
        self.tree.tag_configure("match", background="#1e3a32", foreground="#c8e6d0")
        self.tree.tag_configure("diff", background="#3d3520", foreground="#ffd7a8")
        self.tree.tag_configure("missing", background="#3d2530", foreground="#ffccd5")
        ...
        # 填充数据（前15行）
        for idx, (_, row) in enumerate(result_df.head(15).iterrows()):
            values = []
            for col in columns:
                val = row.get(col, "")
                if pd.isna(val):
            ...
            if tag:
                ...
                self.tree.insert("", "end", values=values, tags=(zebra_tag,))
