...
class QtMainWindow(QMainWindow):
    ...
    def _export_manual_preview(self):
        ...
        ws1.cell(row=2, column=1, value=f"共 {len(df_original)} 行数据，预览前100行")
        ...
        for r_idx, row in enumerate(df_original.head(100).itertuples(index=False), start_row + 1):
        ...
        if clean_rules:
            ...
            for i, rule in enumerate(clean_rules):
                mode_text = f"{rule['column']}: {rule['mode']} 正则'{rule['regex']}'"
                if rule.get('replace'):
        ...
        for c_idx, col in enumerate(df_cleaned.columns, 1):
            ...
            cell.border = thin_border
        for r_idx, row in enumerate(df_cleaned.head(100).itertuples(index=False), start_row + 1):
            for c_idx, value in enumerate(row, 1):
        ...
        if manual_pivot and manual_pivot.get("pivot_column"):
            ...
            if manual_key_cols and manual_val_col:
                ...
                try:
                    ...
                    for r_idx, row in enumerate(pivot_df.head(100).itertuples(index=False), start_row + 1):
                        for c_idx, value in enumerate(row, 1):
