...
class QtResultPreview(QWidget):
    ...
    def _setup_ui(self):
        ...
        layout.addWidget(self.preview_section)  # 不设置stretch，由toggle动态控制
        
        # 底部弹性空间（默认stretch=1，当对账预览收缩时吸收多余空间）
        self._bottom_stretch_index = layout.count()
        layout.addStretch(1)
    ...
    def _toggle_preview(self):
        ...
        if self._preview_expanded:
            # 展开：启用响应式，对账预览占用空间，底部弹性空间不占用
            self.preview_section.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Expanding)
            if parent_layout:
                parent_layout.setStretchFactor(self.preview_section, 1)
                # 底部弹性空间不占用
                if hasattr(self, '_bottom_stretch_index'):
                    item = parent_layout.itemAt(self._bottom_stretch_index)
                    if item:
                        parent_layout.setStretch(self._bottom_stretch_index, 0)
        else:
            # 收缩：让对账预览不占用额外空间，底部弹性空间吸收多余空间
            ...
            if parent_layout:
                ...
                if hasattr(self, '_bottom_stretch_index'):
                    item = parent_layout.itemAt(self._bottom_stretch_index)
                    if item:
                        parent_layout.setStretch(self._bottom_stretch_index, 1)
