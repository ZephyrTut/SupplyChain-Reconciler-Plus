...
class TemplateManagerDialog:
    ...
    @staticmethod
    def _is_user_confirmed(result) -> bool:
        """兼容 ttkbootstrap Messagebox 在不同语言/版本下的返回值"""
        ...
        return text_lower in {"yes", "y", "true", "ok", "okay"} or text in {"确认", "确定", "是", "好", "好的"}
    ...
    def _create_widgets(self):
        ...
        scrollbar = ttk.Scrollbar(list_container, orient="vertical", command=canvas.yview, bootstyle="secondary-round")
    ...
    def _delete_template(self, template: Dict):
        ...
        print(f"[UI] 用户确认结果: {result} (type={type(result)})")

        if self._is_user_confirmed(result):
