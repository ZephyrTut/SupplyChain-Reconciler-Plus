...
class QtConfigPanel(QScrollArea):
    ...
    def _setup_ui(self):
        ...
        # 5. 手工表透视（可折叠）
        self._create_manual_pivot_section()
    ...
    def _create_clean_section(self):
        """创建列数据清洗区（可折叠）"""
        self.clean_section = CollapsibleSection("列数据清洗（可选）", "🧹")
        self.clean_section.set_expanded(False)
        
        # 说明
        hint = QLabel("使用正则表达式清洗手工表列数据（如去除中文、提取数字等）")
        hint.setStyleSheet("color: #666; font-size: 12px;")
        hint.setWordWrap(True)
        self.clean_section.add_widget(hint)
        ...
        self.clean_section.add_widget(self.clean_container)
        ...
        self.clean_section.add_widget(add_clean_btn)
        
        # 预设正则提示
        preset_hint = QLabel("💡 常用正则: [\\u4e00-\\u9fa5]+ (匹配中文)、\\d+ (匹配数字)")
        preset_hint.setStyleSheet("color: #999; font-size: 11px; margin-top: 5px;")
        preset_hint.setWordWrap(True)
        self.clean_section.add_widget(preset_hint)
        
        self.main_layout.addWidget(self.clean_section)
    ...
    def _create_manual_pivot_section(self):
        """创建手工表透视配置区（可折叠）"""
        self.manual_pivot_section = CollapsibleSection("手工表透视（可选）", "🔄")
        self.manual_pivot_section.set_expanded(False)
        
        # 说明
        hint = QLabel("手工表需要透视时使用，可区分出库/入库类型。手工数量 = Σ出库 - Σ入库")
        hint.setStyleSheet("color: #666; font-size: 12px;")
        hint.setWordWrap(True)
        self.manual_pivot_section.add_widget(hint)
        
        # 手工表透视配置组件
        self.manual_pivot_config = ManualPivotConfig()
        self.manual_pivot_config.changed.connect(self._emit_config_changed)
        self.manual_pivot_section.add_widget(self.manual_pivot_config)
        
        self.main_layout.addWidget(self.manual_pivot_section)
    ...
    def set_columns(self, manual_columns: List[str], system_columns: List[str]):
        ...
        # 更新手工表透视配置的列选项
        self.manual_pivot_config.set_columns(manual_columns)
    ...
    def set_manual_unique_values(self, unique_values: Dict[str, List]):
        ...
        for row in self.manual_filter_rows:
            row.update_unique_values(unique_values)
        # 更新手工表透视组件
        self.manual_pivot_config.set_unique_values(unique_values)
    ...
    def get_config(self) -> Dict[str, Any]:
        ...
        # 手工表透视
        manual_pivot = self.manual_pivot_config.get_config()
        if manual_pivot:
            config["manual_pivot"] = manual_pivot
    ...
    def set_config(self, config: Dict[str, Any]):
        ...
        if manual_pivot:
            self.manual_pivot_config.set_config(manual_pivot)
