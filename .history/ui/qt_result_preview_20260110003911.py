...
class SampleDisplay(QFrame):
    ...
    def _setup_ui(self, title: str):
        ...
        self.data_combo = NoScrollComboBox()
        ...
        """)
        self.data_combo.setMaxVisibleItems(15)
    ...
    def set_data(self, df: pd.DataFrame, key_cols: List[str], value_col: str, 
        ...
        if items:
            self.data_combo.addItems(items)
            desc = f"共 {total_count} 条"
            if pivot_info:
                desc += f" | 透视: {pivot_info}"
            self.desc_label.setText(desc)
        else:
            self.data_combo.addItem("（无数据）")
    ...
    def set_pivot_preview(self, pivot_df: pd.DataFrame, manual_pivot: Dict[str, Any], 
        ...
        desc_parts.append(f"📤{', '.join(out_values) if out_values else '无'}")
        desc_parts.append(f"📥{', '.join(in_values) if in_values else '无'}")
        ...
        if pivot_df is not None and not pivot_df.empty:
            ...
            if items:
                self.data_combo.addItems(items)
                desc_parts.append(f"共{len(display_df)}条")
            else:
                self.data_combo.addItem("（无数据）")
        else:
            self.data_combo.addItem("（请先配置）")
    ...
    def set_key_preview(self, df: pd.DataFrame, key_col: str = "__KEY__", 
        ...
        if df is not None and not df.empty and key_col in df.columns:
            ...
            self.data_combo.addItem("（请先配置主键）")
