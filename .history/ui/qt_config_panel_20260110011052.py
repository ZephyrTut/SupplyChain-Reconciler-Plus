...
class ColumnCleanRow(DynamicRow):
    ...
    def _setup_ui(self):
        ...
        col_label = QLabel("列:")
        col_label.setStyleSheet("color: #666; font-size: 11px;")
        ...
        custom_label.setStyleSheet("color: #666; font-size: 11px;")
        row1.addWidget(custom_label)
        
        self.custom_edit = QLineEdit()
        self.custom_edit.setPlaceholderText("输入要去除的内容")
        self.custom_edit.setMaximumWidth(100)
        self.custom_edit.setStyleSheet("font-size: 11px; padding: 2px 4px;")
        self.custom_edit.textChanged.connect(lambda: self.changed.emit())
        row1.addWidget(self.custom_edit, 1)
        
        del_btn = QPushButton("✕")
        del_btn.setFixedSize(20, 20)
        del_btn.setStyleSheet("""
            QPushButton { background-color: #ffebee; color: #f44336; border: none; border-radius: 10px; font-weight: bold; }
