...
class QtMainWindow(QMainWindow):
    ...
    def _on_sheet_changed(self, file_type: str, sheet_name: str):
        ...
        if filepath:
            try:
                ...
                QMessageBox.warning(self, "加载失败", f"无法加载工作表:\n{str(e)}")
    ...
    def _run_comparison(self):
        ...
        try:
            ...
            # 生成主键
            manual_with_key = CompareEngine.make_key(self.manual_df.copy(), manual_key_cols)
            system_with_key = CompareEngine.make_key(self.system_df.copy(), system_key_cols)
            ...
            system_val = "系统总计" if pivot_col else (system_val_col if system_val_col else "")
    ...
    def _on_config_changed(self):
        """配置变更事件"""
        print(f"[DEBUG] _on_config_changed 被调用")  # 调试
        ...
        if self.manual_df is not None and self.system_df is not None:
            ...
            print(f"[DEBUG] 从预览获取的 column_letters: {column_letters}")  # 调试
            if column_letters:
                self.config_panel.update_formula_options(column_letters)
            else:
                print("[DEBUG] column_letters 为空，未调用 update_formula_options")
