...
class QtResultPreview(QWidget):
    ...
    def update_preview(self, manual_df: pd.DataFrame, system_df: pd.DataFrame, 
        ...
        try:
            ...
            # 系统总计
            self.column_letters["系统总计"] = self._excel_col_letter(col_index)
            col_index += 1
            
            # 手工数量
            self.column_letters["手工数量"] = self._excel_col_letter(col_index)
            ...
            for col, letter in sorted(self.column_letters.items(), key=lambda x: x[1]):
                mapping_parts.append(f"{letter} = {col}")
            
            # 添加公式变量说明
            mapping_parts.append("M = 手工数量")
            mapping_parts.append("S = 系统总计")
    ...
    def update_result_preview(self, result_df: pd.DataFrame, pivot_values: List[str], 
        ...
        try:
            ...
            # 生成列字母映射（基于导出顺序，跳过__KEY__）
            self.column_letters.clear()
            col_index = 0
            for col in columns:
                if col != "__KEY__":
                    self.column_letters[col] = self._excel_col_letter(col_index)
                    col_index += 1
            ...
            for pv in pivot_values[:5]:
                letter = self.column_letters.get(pv, "?")
                mapping_parts.append(f"{letter} = {pv}")
            
            manual_letter = self.column_letters.get("手工数量", "M")
            system_letter = self.column_letters.get("系统总计", "S")
            mapping_parts.append(f"{manual_letter} = 手工数量")
            mapping_parts.append(f"{system_letter} = 系统总计")
    ...
    def set_data(self, df: pd.DataFrame, config: Dict[str, Any] = None):
        ...
        for col in display_df.columns:
            if col == "__KEY__":
                ...
                # 透视列、系统总计、手工数量
                letter = self._excel_col_letter(col_index)
