...
class ColumnCleanRow(DynamicRow):
    ...
    def _setup_ui(self):
        ...
        col_label = QLabel("清洗列:")
        col_label.setStyleSheet("color: #333; font-weight: bold;")
        top_row.addWidget(col_label)
        
        self.column_combo = NoScrollComboBox()
        self.column_combo.addItem("(选择列)")
        self.column_combo.addItems(self.columns)
        self.column_combo.currentIndexChanged.connect(lambda: self.changed.emit())
        top_row.addWidget(self.column_combo, 1)
        ...
        rules_container.setStyleSheet("background-color: #fafafa; border-radius: 4px; padding: 5px;")
        rules_layout = QHBoxLayout(rules_container)
        rules_layout.setContentsMargins(8, 5, 8, 5)
        rules_layout.setSpacing(12)
        
        for preset_name in self.CLEAN_PRESETS.keys():
        ...
        self.custom_check = QCheckBox("去除自定义符号:")
        self.custom_check.setStyleSheet("color: #333;")
        self.custom_check.stateChanged.connect(self._on_custom_changed)
        custom_row.addWidget(self.custom_check)
        
        self.custom_edit = QLineEdit()
        self.custom_edit.setPlaceholderText("输入要去除的符号，如: 重复出库")
    ...
    def _on_custom_changed(self, state: int):
        """自定义复选框状态变化"""
        self.custom_edit.setEnabled(state == 2)
        if state != 2:
    ...
    def get_value(self) -> Dict:
        ...
        for preset_name, cb in self._checkboxes.items():
            if cb.isChecked():
                selected_presets.append(preset_name)
                regexes.append(self.CLEAN_PRESETS[preset_name])
        
        # 自定义符号
        custom_text = ""
        if self.custom_check.isChecked() and self.custom_edit.text().strip():
        ...
        return {
            "column": column,
            "mode": "删除匹配",
            "preset": " + ".join(selected_presets),  # 显示用
            "regexes": regexes,  # 多个正则表达式
            "custom": custom_text
        }
    ...
    def set_value(self, value: Dict):
        ...
        for preset_name, cb in self._checkboxes.items():
            cb.setChecked(preset_name in preset_str)
        
        # 恢复自定义符号
        custom = value.get("custom", "")
        if custom:
            self.custom_check.setChecked(True)
            self.custom_edit.setText(custom)
