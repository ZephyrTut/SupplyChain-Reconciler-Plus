...
#### 🎨 UI 优化
- **统一对话框风格**
  - 新增统一样式系统：`DIALOG_STYLE`, `PRIMARY_BTN_STYLE`, `SECONDARY_BTN_STYLE`, `DANGER_BTN_STYLE`
  - 新增便捷对话框函数：`show_info()`, `show_warning()`, `show_error()`, `show_confirm()`
  - 所有对话框使用白色背景、一致的按钮样式和配色
  - 移除对 `QMessageBox` 的直接依赖

- **对话框尺寸优化**
  - 减小按钮 padding（10px 24px → 6px 16px）
  - 减小对话框整体尺寸，更紧凑美观
  - 统一边距和间距（20px 边距，15px 间距）

- **qt-material 主题调整**
  - 设置 `invert_secondary=True` 解决标题栏颜色问题
  - 覆盖对话框样式确保浅色背景统一

- **保存模板功能增强**
  - 保存模板对话框新增已有模板下拉选择
  - 支持覆盖已有模板或输入新名称创建
  - 提示文字："💡 选择已有模板将覆盖，或输入新名称创建"
...
#### 🔧 核心功能改进
- **动态公式变量系统**
  - 公式变量从固定 M/S 改为动态列字母（B, C, D...）
  - 列顺序调整为：KEY → 透视列 → 系统总计 → 手工数量 → 差值 → 比对状态
  - 公式快速选择根据实际数据动态更新选项

- **筛选条件持久化**
  - 模板保存/加载时包含手工表和系统表筛选条件
...
|-------------|-------|-------|---------|---------|-----|---------|
| A001_SKU123 | 80    | 20    | 100     | 100     | 0   | ✓       |
| A002_SKU456 | 180   | 15    | 195     | 200     | 5   | ↕       |
| A003_SKU789 | -     | -     | -       | 150     | 150 | ✗       |
```

**列顺序**: KEY → 透视列（按名称排序）→ 系统总计 → 手工数量 → 差值 → 比对状态
