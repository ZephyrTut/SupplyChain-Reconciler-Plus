...
- [ 模板默认来源](#模板默认来源) - 默认模板与本机路径

---

##  模板默认来源

首次运行时，应用会把仓库内的 templates.json 作为默认模板写入用户配置目录。

在 Windows 的 CMD 中可执行以下命令查找本机模板文件：

```bat
dir "%APPDATA%\SupplyChain-Reconciler-Plus\templates.json" /s
```
