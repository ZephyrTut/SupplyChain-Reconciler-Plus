...
| manual_filter_exceptions | 手工表例外保留条件 |
| system_filter_exceptions | 系统表例外保留条件 |
...
当满足以下条件时，加载模板后会自动回写模板配置：

- 手工表与系统表均已导入
- 当前数据导致模板配置发生可计算更新（如透视值列表、可用列映射等）

为避免误覆盖：
- 若主键映射或数值列配置不完整，不会自动回写
- 仅在配置完整且有实际变化时才会写入

目的：确保模板与当前两表数据保持同步，减少下次重复修正。
...
首次运行时，如果用户目录不存在模板文件，应用会从仓库内的 templates.json
写入默认模板列表。
...
- 当前版本模板目录为：`SupplyChain-Reconciler-Plus`
- 如果检测到旧目录 `SupplyChain-Reconciler`，会自动迁移 `templates.json` / `config.json`
- 兼容两种模板JSON结构：
  - `[{...}, {...}]`
  - `{ "templates": [{...}] }`
