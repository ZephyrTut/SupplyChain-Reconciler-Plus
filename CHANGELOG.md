...
### ✨ 新增与改进
- ✅ 打包流程统一使用 `.spec`（release 与本地行为一致）
- ✅ 增加无关依赖过滤（减少打包体积）
- ✅ `templates.json` 随安装包保留并自动初始化到用户目录
- ✅ 本地可直接生成与 release 命名一致的 `zip + sha256`

### 📦 打包优化
- ✅ 优化后 EXE 体积稳定在 60+MB 区间（干净环境）
- ✅ 清理环境污染导致的额外依赖打包问题
...
| 版本 | 发布日期 | 支持状态 |
|------|---------|---------|
| v1.5.0 | 2026-05-01 | 🟢 当前版本 |
| v1.4.4 | 2026-03-16 | 🟡 维护中 |
| v1.4.3 | 2026-01-11 | 🟡 维护中 |
